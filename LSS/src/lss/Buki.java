...
//神話武器
        //アインハザードの閃光                                                   +1強化毎に[近距離追加ダメージ+4][近距離命中+1][近距離クリティカル+2%][技術命中+1][精霊命中+1][恐怖命中+1][魔法ダメ上昇][武器ダメージ-イミューン無視+5%]増加
        //アインハザードの閃光(対モンスター範囲魔法)とアインハザードの一撃(対人魔法+2秒ホールド)は未実装 INT&SPに魔法ダメージは依存しない
        if (name.equals("アインハザードの閃光") || name.equals("アインハザードの閃光(アインハザードの閃光発動)")) {
            if (enchant >= 0) {
                                                                                //武器ダメージ イミューン無視+20%(未実装)
            op2.DMG_SHORT += (enchant*4);                                       //近距離追加ダメージ+4x強化数
        ...
        //グランカインの審判                                                     +1強化毎に[近距離追加ダメージ+4][近距離命中+1][近距離クリティカル+2%][技術命中+1][魔法ダメ上昇][武器ダメージ-イミューン無視+5%]増加
        //グランカインの咆哮(対モンスター範囲魔法)とグランカインの審判(対人魔法+2秒ホールド)は未実装 INT&SPに魔法ダメージは依存しない
        if (name.equals("グランカインの審判") || name.equals("グランカインの審判(グランカインの審判発動)")) {
            if (enchant >= 0) {
                                                                                //武器ダメージ イミューン無視+20%(未実装)
            op2.DMG_SHORT += (enchant*4);                                       //近距離追加ダメージ+4x強化数
            op2.HIT_SHORT += enchant;                                           //近距離命中+1x強化数
        ...
        //エヴァの誓約                                                           +1強化毎に[近距離追加ダメージ+4][近距離命中+1][魔法ダメージ+1%][魔法命中+2][SP+2][武器ダメージ-イミューン無視+5%]増加
        //エヴァの審判(効果不明)は未実装
        if (name.equals("エヴァの誓約")) {
            if (enchant >= 0) {
                                                                                //武器ダメージ-イミューン無視+20%(未実装)
            op2.DMG_SHORT += (enchant*4);                                       //近距離追加ダメージ+4x強化数
            op2.HIT_SHORT += enchant;                                           //近距離命中+1x強化数
        ...
        //マーブルの苦悩                                                         +1強化毎に[近距離追加ダメージ+4][近距離命中+1][近距離クリティカル+2%][恐怖命中+1][武器ダメージ-イミューン無視+5%]増加
        //マーブルの伝言(効果不明)は未実装
        if (name.equals("マーブルの苦悩")) {
            if (enchant >= 0) {
                                                                                //武器ダメージ-イミューン無視+20%(未実装)
            op2.DMG_SHORT += (enchant*4);                                       //近距離追加ダメージ+4x強化数
            op2.HIT_SHORT += enchant;                                           //近距離命中+1x強化数
            op2.CRI_SHORT += (enchant*2);                                       //近距離クリティカル+2x強化数
            op2.ailment[HIT_TERROR] += enchant;                                 //恐怖命中+1x強化数
            }
        }
