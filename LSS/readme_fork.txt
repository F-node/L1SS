...
料理追加
	幻想のショートケーキ(KR版)
		ダメージ減少+5/EXP10%
修正
	武器のツールチップ(死神の剣/アインハザードの閃光/グランカインの審判等の基本ダメージ表記)

2020/01/07
Update 3.7.2
LSS/data/E.zip
修正
	原因:装備データーの【特殊=】のコメントを読み込んでエラーを起こす
	特殊=コメントを削除
対象装備
	ディヴァインエルヴンシールド
	バフォメットアーマー
	...
	ギルタスのソード
	咆哮のデュアルブレード
	パプリオンのデュアルブレード(REMASTERED)(パプリオンの魂発動)
	パプリオンのデュアルブレード(REMASTERED)
対象外装備
	エヴァのシールド(ヘイストの処理は実装されている為)
修正
	ディヴァインプロテクションクロークの装備クラス
...
LSS/src/lss/UI.java
修正
	2019/11/20Update HW/EW/BW/SFの仕様変更にて武器からキャラに対し効果が発生しダメージ計算される様になった
	しかし旧計算方法(武器に対し効果が発生)でダメージ計算されているとの情報があり項目追加(仕様確認後修正)
	レジェンド武器の+10の魔法追加ダメージを削除
	武器の変数に対してコメント追加
...
LSS/src/lss/Buki.java
装備追加
	2019/10/30 Update(KR)で実装されたドラゴンの武器(8種類)
	追加魔法は準レジェンド武器の魔法と同一で仮設定
	ダブルヒット発動確率増加/弱点露出確率増加は未実装
	フェンサーはFで仮設定

	ヴァラカスのソード
	君主/エルフ/フェンサー
	ダメージ33/22 非損傷
	STR+1 CON+1 命中+13 追加ダメージ+10 技術/精霊/恐怖命中+2
	+8から+1ごとに技術/精霊/恐怖命中+1(最大+3)
	+10で魔法発動:ヴァラカスの魂

	ヴァラカスの両手剣
	ナイト
	ダメージ31/39 非損傷
	STR+2 命中+9 追加ダメージ+14 技術命中+5
	+8から+1ごとに技術命中+1(最大+3)
	+10で魔法発動:ヴァラカスの魂
	...
	エルフ
	ダメージ25/22
	DEX+2 命中+7 追加ダメージ+10 ダメージリダクション無視+2
	+8から+1ごとに精霊命中+1/ダメージリダクション無視+1(最大+3)
	+10で魔法発動:パプリオンの魂

	パプリオンのデュアルブレード
	ダークエルフ
	ダメージ26/18
	STR+2 命中+8 追加ダメージ+7 精霊命中+2
	+8から+1ごとに精霊命中+1(最大+3)
	エンチャント数値に基づいてダブルヒット発動確率増加
	+10で魔法発動:パプリオンの魂
	ダブルヒット発動確率増加は未実装
	...
	戦士
	ダメージ33/36 非損傷
	STR+1 WIS+1 命中+5 追加ダメージ+9 恐怖命中+2
	+8から+1あたり恐怖命中+1(最大+3)
	+10で魔法発動:アンタラスの魂
	...
	ウィザード
	ダメージ10/10 非損傷
	INT+1 命中+9は SP+5  魔法命中+1
	+8から+1ごとにSP+1/魔法命中+1(最大+3)
	+10で魔法発動:アンタラスの魂(エンチャント数値に基づいて発動確率増加)

	リンドビオルのチェーンソード
	ドラゴンナイト
	ダメージ30/24 非損傷
	STR+1 命中+8 追加ダメージ+11 秘技命中+5
	+8から+1ごとに秘技命中+1(最大+3)
	+10で魔法発動:リンドビオルの魂
	弱点露出確率増加は未実装
	...
	イリュージョニスト
	ダメージ36/36 非損傷
	INT+2 SP+1 HP+50 秘技命中+5
	+8から+1ごとにSP+1/秘技命中+1(最大+3)
	+10で魔法発動:リンドビオルの魂(エンチャント数値に基づいて発動確率増加)

装備削除
	幸運のグレートソード(2019_09_旧仕様)
	幸運のスピアー(2019_09_旧仕様)
	幸運のランス(2019_09_旧仕様)
	幸運のバトルアックス(2019_09_旧仕様)
	幸運のマナスタッフ(2019_09_旧仕様)
	幸運のデュアルブレード(2019_09_旧仕様)
	幸運のチェーンソード(2019_09_旧仕様)
	幸運のキーリンク(2019_09_旧仕様)

2019/12/16
Update 3.6.8
LSS/src/lss/Calculator.java
スキル修正
	2019/10/30 Update(KR)
...
LSS/src/lss/Buki.java
	...
	[テンペストアックス]
	+8以降強化1毎に恐怖命中+1(+10強化まで適用)

	[タイタンの憤怒]
	基本恐怖命中+5 +1以降強化1毎に恐怖命中+1(+5強化まで適用)
...
LSS/src/lss/UI.java
	...
	バフォメット  最大HP+120 追加防御力+4 ダメージ低下+4 近距離ダメージ+3 近距離命中+3 恐怖命中+5
